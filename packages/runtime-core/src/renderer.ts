...
function baseCreateRenderer(
  ...
  const mountComponent: MountComponentFn = (
    ...
    if (!(__COMPAT__ && compatMountInstance)) {
      ...
      // TODO 运行 setup 函数，兼容 options api，将模板编译为 render 函数
  ...
  const setupRenderEffect: SetupRenderEffectFn = (
    ...
    const componentUpdateFn = () => {
      if (!instance.isMounted) {
        ...
        // TODO mounted 生命周期钩子 mounted hook 异步执行 then(() => {onMounted})
        if (m) {
    ...
    // TODO effect.run 执行 componentUpdateFn 方法，初始化 instance.update 和 instance.job
    const update = (instance.update = effect.run.bind(effect))
    const job: SchedulerJob = (instance.job = effect.runIfDirty.bind(effect))
    job.i = instance
    job.id = instance.uid
    // TODO queueJob 队列执行 job 及 instance.job ->  effect.runIfDirty -> effect.run
  ...
  // TODO 简称 p/um/m/r/mt/mc/pc/pbc/n/o
